...
def generate_topic_heatmap(Topic_N, interval_width = 24, user = "defaultValue"):
    ...
    if (user != "defaultValue"):
        ...
        user = ""
    TemporalDistr = []
    for i in np.arange(0,24/interval_width):
        TemporalDistr.append([])
    datelist = t1["date"].tolist()
    K= len(datelist)
    for i in range(0,K):
         t = datelist[i].hour
         TemporalDistr[t/interval_width].append(t1.iloc[i])
    
    for t in np.arange(0,24/interval_width):
        ...
        mymap = "mymap_vanc_"+ str(Topic_N)+user+"_interval_width_"+str(interval_width)+"_hour_"+str(t*interval_width)+".html"
        gmap.draw(mymap)
...
def generate_topic_heatmap_weekdays(Topic_N, user = "defaultValue"):
    ...
    for t in np.arange(0,7):
        ...
        gmap = gmplot.GoogleMapPlotter(49.2827, -123.1207, 11)
        gmap.heatmap(lats, longs,dissipating=True,radius=40)
        mymap = "mymap_vanc_"+  str(Topic_N)+user +abrev_num[t][0]+".html"
        gmap.draw(mymap)
